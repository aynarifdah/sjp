...
public function distribusi($bulan=Null,$tahun=Null,$kecamatan=Null,$kelurahan=Null,$pilih=0){
  ...
  foreach ($query as $res) {
    $result[] = [$res["rs"], (integer)$res["jumlah"] ];
  }
  // $result[] = $query;
  // print_r($result);die;
...
public function jumlah_kunjungan_bulan($bulan=Null,$tahun=Null,$kecamatan=Null,$kelurahan=Null){
  ...
  $januari  = isset($hasil[0]["januari"]) ? (integer)$hasil[0]["januari"]  : 0;
  $februari = isset($hasil[0]["februari"])? (integer)$hasil[0]["februari"] : 0;
  $maret    = isset($hasil[0]["maret"])   ? (integer)$hasil[0]["maret"]    : 0;
  $april    = isset($hasil[0]["april"])   ? (integer)$hasil[0]["april"]    : 0;
  $mei      = isset($hasil[0]["mei"])     ? (integer)$hasil[0]["mei"]      : 0;
  $juni     = isset($hasil[0]["juni"])    ? (integer)$hasil[0]["juni"]     : 0;
  $juli     = isset($hasil[0]["juli"])    ? (integer)$hasil[0]["juli"]     : 0;
  $agustus  = isset($hasil[0]["agustus"]) ? (integer)$hasil[0]["agustus"]  : 0;
  $september= isset($hasil[0]["september"])? (integer)$hasil[0]["september"] : 0;
  $oktober  = isset($hasil[0]["oktober"]) ? (integer)$hasil[0]["oktober"]  : 0;
  $november = isset($hasil[0]["november"])? (integer)$hasil[0]["november"] : 0;
  $desember = isset($hasil[0]["desember"])? (integer)$hasil[0]["desember"] : 0;
...
public function trend_pasien($bulan=Null,$tahun=Null,$kecamatan=Null,$kelurahan=Null){
  ...
  for ($i=0; $i <= 4; $i++) { 
    ...
    $result[$i][] = (integer)$query[0]['jumlah'];
...
function select_pengajuan_sjp_all($id_status_pengajuan = null, $puskesmas=Null, $rumahsakit=Null, $status=Null, $cari=Null){
  $this->db->select('CONCAT(sjp.alamat, ",", " RT. ", sjp.rt, " RW. ", sjp.rw, " Kel. ", sjp.kd_kelurahan, " Kec. ", sjp.kd_kecamatan) AS alamatpasien, pp.tanggal_pengajuan, pp.nama_pemohon, pp.jenis_kelamin as jkpemohon, pp.telepon as telpemohon, pp.whatsapp as wapemohon, pp.email as email, pp.alamat as alamatpemohon, pp.kd_kelurahan as kelpemohon, pp.kd_kecamatan as kecpemohon, pp.rt as rtpemohon, pp.rw as rwpemohon, pp.status_hubungan, pp.nama_pejabat_satu, pp.nip_pejabat_satu, sjp.*, sp.status_pengajuan, pp.id_status_pengajuan ,rs.nama_rumah_sakit as nm_rs');
  $this->db->from('permohonan_pengajuan pp');
  $this->db->join('sjp', 'sjp.id_pengajuan = pp.id_pengajuan', 'left');
  $this->db->join('rumah_sakit rs', 'sjp.id_rumah_sakit = rs.id_rumah_sakit', 'left');
  $this->db->join('status_pengajuan sp', 'sp.id_statuspengajuan = pp.id_status_pengajuan', 'left');
...
public function view_permohonansjp_pus($id_jenissjp,$puskesmas=Null,$rumahsakit=Null, $status=Null, $cari=Null, $id_join=null, $id_instansi=null){
