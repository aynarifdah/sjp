...
class M_SJP extends CI_Model
{
  ...
  public function jenisjaminan($all = null)
  {
    $this->db->select('*');
  ...
  function select_pengajuan_sjp_all($limit = null, $puskesmas = Null, $rumahsakit = Null, $status = Null, $cari = Null, $mulai = Null, $akhir = null, $jaminan = null,$kelurahan = null, $status_jkn = null, $offset = 0)
  {
    $this->db->select('CONCAT(sjp.alamat, ",", " RT. ", sjp.rt, " RW. ", sjp.rw, " Kel. ", sjp.kd_kelurahan, " Kec. ", sjp.kd_kecamatan) AS alamatpasien, pp.tanggal_pengajuan, pp.tanggal_selesai, pp.nama_pemohon, pp.jenis_kelamin as jkpemohon, pp.telepon as telpemohon, pp.whatsapp as wapemohon, pp.email as emailpemohon, pp.alamat as alamatpemohon, pp.kd_kelurahan as kelpemohon, pp.kd_kecamatan as kecpemohon, pp.rt as rtpemohon, pp.rw as rwpemohon, pp.status_hubungan, pp.nama_pejabat_satu, pp.nip_pejabat_satu, sjp.*, sp.status_pengajuan, pp.id_status_pengajuan ,rs.nama_rumah_sakit as nm_rs, js.nama_jenis, sh.id as id_hubungan, pus.nama_puskesmas');
    $this->db->from('permohonan_pengajuan pp');
    $this->db->join('sjp', 'sjp.id_pengajuan = pp.id_pengajuan', 'left');
    $this->db->join('rumah_sakit rs', 'sjp.id_rumah_sakit = rs.id_rumah_sakit', 'left');
    $this->db->join('status_pengajuan sp', 'sp.id_statuspengajuan = pp.id_status_pengajuan', 'left');
    ...
    if (!empty($status_jkn)) {
      $this->db->where('sjp.status_jkn =', $status_jkn);
    }
    if (!empty($cari)) {
  ...
  public function count_filtered_sjp($puskesmas = Null, $rumahsakit = Null, $status = Null, $cari = Null, $mulai = Null, $akhir = null, $jaminan = null, $status_jkn = null)
  {
    $this->db->from('permohonan_pengajuan pp');
    $this->db->join('sjp', 'sjp.id_pengajuan = pp.id_pengajuan', 'left');
    $this->db->join('rumah_sakit rs', 'sjp.id_rumah_sakit = rs.id_rumah_sakit', 'left');
    ...
    if (!empty($puskesmas)) $this->db->where('pus.id_puskesmas', $puskesmas);
    if (!empty($jaminan)) $this->db->where('sjp.jenis_sjp', $jaminan);
    if (!empty($mulai)) $this->db->where('pp.tanggal_pengajuan >=', $mulai);
    if (!empty($akhir)) {
        $tanggal = date('Y-m-d', strtotime($akhir));
        $this->db->where('pp.tanggal_pengajuan <=', $tanggal . ' 23:59:59');
    }
    if (!empty($rumahsakit)) $this->db->where('rs.id_rumah_sakit', $rumahsakit);
    if (!empty($status)) $this->db->where('pp.id_status_pengajuan', $status);
    if (!empty($status_jkn)) $this->db->where('sjp.status_jkn', $status_jkn);
    if (!empty($cari)) {
        $this->db->like('CONCAT(pp.nama_pemohon,sjp.nama_pasien,sjp.nik,sjp.kd_kelurahan,sjp.kd_kecamatan,pp.email,pp.status_hubungan,sp.status_pengajuan,rs.nama_rumah_sakit,sjp.email,sjp.pekerjaan)', $cari);
    }
  ...
  public function getpersetujuansjpdinas($puskesmas = Null, $rumahsakit = Null, $status = Null, $cari = Null, $mulai = Null, $akhir = Null, $jaminan = Null)
  {
    $this->db->select('CONCAT(sjp.alamat, ",", " RT. ", sjp.rt, " RW. ", sjp.rw, " Kel. ", sjp.kd_kelurahan, " Kec. ", sjp.kd_kecamatan) AS alamatpasien, pp.tanggal_pengajuan, pp.tanggal_selesai, pp.nama_pemohon, pp.jenis_kelamin as jkpemohon, pp.telepon as telpemohon, pp.whatsapp as wapemohon, pp.email as email, pp.alamat as alamatpemohon, pp.kd_kelurahan as kelpemohon, pp.kd_kecamatan as kecpemohon, pp.rt as rtpemohon, pp.rw as rwpemohon, pp.status_hubungan, pp.nama_pejabat_satu, pp.nip_pejabat_satu, sjp.*, sp.status_pengajuan, pp.id_status_pengajuan ,rs.nama_rumah_sakit as nm_rs, js.nama_jenis');
    $this->db->from('sjp');
    $this->db->join('permohonan_pengajuan pp', 'pp.id_pengajuan = sjp.id_pengajuan', 'left');
  ...
  public function getKelurahanBySession()
  {
    $this->db->select('key_wilayah as id, kelurahan as nama');
    $this->db->from('kelurahan');
    $this->db->where('key_wilayah', $this->session->userdata('id_join'));
    $query = $this->db->get()->result_array();
    return $query;
  }
  ...
  public function getditolaksjpdinas($puskesmas = Null, $rumahsakit = Null, $status = Null, $cari = Null, $mulai = Null, $akhir = Null, $jaminan = Null)
  {
    $this->db->select('CONCAT(sjp.alamat, ",", " RT. ", sjp.rt, " RW. ", sjp.rw, " Kel. ", sjp.kd_kelurahan, " Kec. ", sjp.kd_kecamatan) AS alamatpasien, pp.tanggal_pengajuan, pp.tanggal_selesai, pp.nama_pemohon, pp.jenis_kelamin as jkpemohon, pp.telepon as telpemohon, pp.whatsapp as wapemohon, pp.email as email, pp.alamat as alamatpemohon, pp.kd_kelurahan as kelpemohon, pp.kd_kecamatan as kecpemohon, pp.rt as rtpemohon, pp.rw as rwpemohon, pp.status_hubungan, pp.nama_pejabat_satu, pp.nip_pejabat_satu, sjp.*, sp.status_pengajuan, pp.id_status_pengajuan ,rs.nama_rumah_sakit as nm_rs, js.nama_jenis');
    $this->db->from('sjp');
    $this->db->join('permohonan_pengajuan pp', 'pp.id_pengajuan = sjp.id_pengajuan', 'left');
  ...
  public function getditolaksjpdinsos($puskesmas = Null, $rumahsakit = Null, $status = Null, $cari = Null, $mulai = Null, $akhir = Null, $jaminan = Null)
  {
    $this->db->select('CONCAT(sjp.alamat, ",", " RT. ", sjp.rt, " RW. ", sjp.rw, " Kel. ", sjp.kd_kelurahan, " Kec. ", sjp.kd_kecamatan) AS alamatpasien, pp.tanggal_pengajuan, pp.tanggal_selesai, pp.nama_pemohon, pp.jenis_kelamin as jkpemohon, pp.telepon as telpemohon, pp.whatsapp as wapemohon, pp.email as email, pp.alamat as alamatpemohon, pp.kd_kelurahan as kelpemohon, pp.kd_kecamatan as kecpemohon, pp.rt as rtpemohon, pp.rw as rwpemohon, pp.status_hubungan, pp.nama_pejabat_satu, pp.nip_pejabat_satu, sjp.*, sp.status_pengajuan, pp.id_status_pengajuan ,rs.nama_rumah_sakit as nm_rs, js.nama_jenis');
    $this->db->from('sjp');
    $this->db->join('permohonan_pengajuan pp', 'pp.id_pengajuan = sjp.id_pengajuan', 'left');
    ...
    if (!empty($jaminan)) {
      $this->db->where_in('jenis_sjp', $jaminan);
    }
    if (!empty($mulai)) {
      $this->db->where('pp.tanggal_pengajuan >=', $mulai);
    }
